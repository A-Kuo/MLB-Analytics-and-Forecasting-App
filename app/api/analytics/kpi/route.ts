...
import { getAggregateKpi } from "@/lib/db/analytics";
...
export async function GET(request: Request) {
  try {
    ...
    const value = await getAggregateKpi(playerIds, metric, group, startYear, endYear);
    return NextResponse.json({ data: value }, { status: 200 });
  } catch (error) {
    console.error("Aggregate KPI fetch error:", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to fetch aggregate KPI" },
      { status: 500 },
    );
  }
}
