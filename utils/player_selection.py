"""Pure, Streamlit-free player-selection logic: the shared per-player label
and the hitting/pitching group used to pick which metric list Aggregate
KPI/Trend/Forecast show.
...
from utils.positions import format_position_tag


def player_flag_label(bio: dict) -> str:
    """"[SS] Player Name (2015–2020)" -- the shared naming convention used
    in the player-selection multiselect (both its option list and its
    native selected-item pills) and the portrait wall. Multi-position
    players render as "[SS, 2B] Name (...)"; a player with more than one
    active-year span (left and later rejoined this team) renders as
    "Name (2015–2017, 2019–present)".
    """
