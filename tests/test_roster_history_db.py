...
def test_schema_file_exists_at_the_expected_path():
    assert roster_history_db.SCHEMA_PATH.is_file()
...
def test_normalize_rewrites_bare_postgresql_scheme_to_psycopg3():
    ...
    assert roster_history_db.normalize_database_url("postgresql://u:p@host/db").startswith(
        "postgresql+psycopg://"
    )


def test_normalize_rewrites_legacy_postgres_scheme():
    assert roster_history_db.normalize_database_url("postgres://u:p@host/db") == (
        "postgresql+psycopg://u:p@host/db"
    )


def test_normalize_preserves_the_rest_of_the_url():
    normalized = roster_history_db.normalize_database_url("postgresql://u:p@host/db?sslmode=require")
...
def test_normalize_leaves_an_already_correct_url_alone():
    ...
    assert roster_history_db.normalize_database_url(url) == url
...
def test_normalize_accepts_a_full_toml_block():
    ...
    assert roster_history_db.normalize_database_url(block) == "postgresql+psycopg://u:p@host/db"


def test_normalize_ignores_a_toml_block_without_a_url_key():
    assert roster_history_db.normalize_database_url("[connections.postgresql]\n") == (
...
def test_resolve_prefers_the_environment_variable(monkeypatch):
    ...
    assert roster_history_db.resolve_database_url() == "postgresql+psycopg://env-host/db"
...
def test_resolve_falls_back_to_secrets_file(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(roster_history_db, "SECRETS_PATH", secrets)
    assert roster_history_db.resolve_database_url() == "postgresql+psycopg://file-host/db"
...
def test_resolve_returns_none_when_secrets_file_is_malformed(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(roster_history_db, "SECRETS_PATH", secrets)
    assert roster_history_db.resolve_database_url() is None


def test_resolve_returns_none_when_nothing_is_configured(monkeypatch, tmp_path):
    monkeypatch.delenv("DATABASE_URL", raising=False)
    monkeypatch.setattr(roster_history_db, "SECRETS_PATH", tmp_path / "missing.toml")
    assert roster_history_db.resolve_database_url() is None
