"""Pre-fits the SVR/Huber/GaussianProcess trajectory ensemble server-side and
returns a JSON-ready payload, so consumers (the Streamlit dashboard) only
ever render a trajectory -- they never fit a model themselves.
...
from clients import mlb_client, statcast_client
...
from filters import metrics_for_group
from regression import fit_trajectory_ensemble
from transform import batted_balls_dataframe, game_log_dataframe, pitches_dataframe, schedule_dataframe
...
def compute_hitter_trajectory(player_id: int, season: int, metric: str) -> dict:
    ...
    game_log = game_log_dataframe(mlb_client.get_game_log(player_id, season, "hitting"))
    ...
    batted_balls = batted_balls_dataframe(statcast_client.get_batter_batted_balls(player_id, season))
...
def compute_pitcher_trajectory(player_id: int, season: int, fallback_metric: str = "era") -> dict:
    ...
    """
    pitches = pitches_dataframe(statcast_client.get_pitcher_pitches(player_id, season))
    ...
    game_log = game_log_dataframe(mlb_client.get_game_log(player_id, season, "pitching"))
...
def compute_team_trajectory(team_id: int, season: int, mode: str) -> dict:
    schedule = schedule_dataframe(mlb_client.get_schedule(team_id, season), team_id)
